...
class APIVersionMiddleware:
    ...
    def __call__(self, request: HttpRequest) -> HttpResponse:
        ...
        # Store version info on request using monkey patching
        # We can't use setattr for Django request objects
        request.api_version = version  # type: ignore
        request.api_version_source = source  # type: ignore
        ...
        if sunset_date and isinstance(sunset_date, datetime):
    ...
    def process_view(
        ...
        view_kwargs["api_version"] = request.api_version  # type: ignore

        # Log version usage
        self._logger.info(
            "api_version_used",
            version=getattr(request, "api_version", DEFAULT_VERSION),
            source=getattr(request, "api_version_source", "unknown"),
            path=request.path,
    ...
    def _get_accept_version(self, request: HttpRequest) -> str:
        ...
        Returns:
            The extracted API version or empty string
        """
