    ...
    async with aiohttp.ClientSession() as session:
        ...
        async with session.post(url, json=payload, headers=headers, auth=auth) as response:
            if response.status != 200:
                ...
                logger.error(f"LLM API error {response.status}: URL: {url}, Response: {error_text}")
                raise Exception(f"API returned error: {response.status}, {error_text}")
            
            logger.info(f"Successfully connected to LLM API, processing streaming response")
                ...
                if line:
                    ...
                    try:
                        ...
                        if accumulated_content == "":
                            logger.info(f"First response chunk format: {json.dumps(chunk_data)}")
                        ...
                        # Ollama format
                            ...
                            if content == "" and "done" in chunk_data and chunk_data["done"]:
                                if chunk_data.get("done_reason") in ["load", "stop"]:
                                    logger.info(f"Received Ollama message with done_reason: {chunk_data.get('done_reason')}")
                                    # This is an initialization or completion message, not actual content
                                    reason = chunk_data.get("done_reason", "unknown")
                                    
                                    if reason == "load":
                                        await update_message_status(ai_message, "Model is loading...")
                                    elif reason == "stop":
                                        # This is the normal completion message, we should have content by now
                                        if accumulated_content:
                                            await update_message_status(ai_message, "Processing complete")
                                        else:
                                            await update_message_status(ai_message, "No content generated")
                                    continue
                        ...
                        if content is None and accumulated_content == "":
                            logger.warning(f"Unknown response format, raw chunk: {line_text}")
                        ...
                        if content:
                            ...
                            await update_message_content(ai_message, accumulated_content)
                            
                            # Update status occasionally to show progress
                            if len(accumulated_content) % 100 == 0:
                                await update_message_status(ai_message, "Still generating...")
                                    
                    except json.JSONDecodeError:
                        logger.warning(f"Could not parse API response as JSON: {line_text}")

            # After processing all chunks, check if we got any content
            if accumulated_content == "":
                logger.warning("No content was extracted from any chunks, using raw response as fallback")
                ...
                if "done_reason" in full_response and "load" in full_response:
                    ...
                    try:
                        ...
                        import re
                        json_objects = re.findall(r'\{.*?"response":[^{]*?\}', full_response, re.DOTALL)
                        
                        for json_str in json_objects:
                            try:
                                data = json.loads(json_str)
                                if "response" in data and data["response"]:
                                    await update_message_content(ai_message, data["response"])
                                    return
                            except:
                                pass
                ...
                try:
                    ...
                    cleaned_response = full_response.replace("data: ", "").replace("[DONE]", "")
                    ...
                    if extracted_text:
                        ...
                        await update_message_content(ai_message, "API responded but format was unrecognized. Raw response: " + cleaned_response[:500])
                except Exception as e:
                    logger.error(f"Failed to extract content from full response: {e}")
                    await update_message_content(ai_message, "Failed to parse API response")
