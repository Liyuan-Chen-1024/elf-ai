...
class MediaSettings(BaseSettings):
    ...
    MEDIA_ROOT: Path = Field(
        default=Path("/data"), description="Root directory for all media files"
    )
    TV_SHOWS_DIR: Path = Field(
        default=Path("/data/tv"), description="Directory for TV show downloads"
    )
    MOVIES_DIR: Path = Field(
        default=Path("/data/movies"), description="Directory for movie downloads"
    )

    # Transmission settings
    TX_HOST: str = Field(default="localhost", description="Transmission RPC host")
    TX_PORT: int = Field(default=9091, description="Transmission RPC port")
    TX_USERNAME: Optional[str] = Field(
        default=None, description="Transmission RPC username"
    )
    TX_PASSWORD: Optional[str] = Field(
        default=None, description="Transmission RPC password"
    )

    # Download settings
    MIN_DOWNLOAD_RATE: int = Field(
        default=150, description="Minimum acceptable download rate in KB/s"
    )
    RATE_RATIO_THRESHOLD: float = Field(
        default=0.5, description="Threshold ratio compared to average download rate"
    )
    MAX_CONCURRENT_DOWNLOADS: int = Field(
        default=3, description="Maximum number of concurrent downloads"
    )
    ...
    MOVIE_FILENAME_TEMPLATE: str = Field(
        default="{movie_name} ({year})", description="Template for movie filenames"
    )
