...
def find_and_process_new_epguide_keys(activate: bool = False) -> List[str]:
    """Find and process new epguide keys.

    Args:
        activate: Whether to activate the discovered shows

    Returns:
        List of processed show keys
    """
    ...
    try:
        ...
        # Extract show keys using regex - fix the escape sequence
...
def get_episode_info(
    ...
    try:
        ...
        # Handle the episode response from the API
        if "episode" in response:
            episode_data = response["episode"]
            # Ensure the episode data matches our EpisodeDict structure
            if isinstance(episode_data, dict) and all(
                k in episode_data for k in ["season", "number"]
            ):
                # Convert to our EpisodeDict format
                return cast(
                    EpisodeDict,
                    {
