...
def delete_empty_directories(path: str, dry_run: bool = False) -> int:
    deleted_count = 0
    for root, dirs, _ in os.walk(path, topdown=True):
        for name in dirs:
            path_name = os.path.join(root, name)
...
def delete_small_video_files(
    path: str, min_size: int = 100000000, dry_run: bool = False
) -> int:
...
def get_movie_folder(keep: bool = False) -> str:
    storage = random.choice(settings.STORAGE)
    if keep:
        storage = os.path.join(storage, "keep/")
