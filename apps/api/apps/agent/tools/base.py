...
class BaseTool(abc.ABC):
    ...
    def __init__(self):
        ...
        if not hasattr(self, 'name') or not self.name:
            raise ValueError(f"Tool {self.__class__.__name__} must define a 'name' class variable")
        
        if not hasattr(self, 'description') or not self.description:
            raise ValueError(f"Tool {self.__class__.__name__} must define a 'description' class variable")
        
        if not hasattr(self, 'category') or not self.category:
            raise ValueError(f"Tool {self.__class__.__name__} must define a 'category' class variable")
