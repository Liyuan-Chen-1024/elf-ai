    ...
    def parse_query(self, query: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for tool_info in selected_tools:
                ...
                logger.info(f"Tool selected: {tool_name} (confidence: {confidence}) - {reasoning}")
...
Response:"""
    ...
    def _generate_parameter_extraction_prompt(self, tool: BaseTool, query: str) -> str:
        ...
        for param in tool.parameters:
            ...
            enum_str = f" (possible values: {', '.join(param.enum)})" if param.enum else ""
            
            params_info.append(f"- {param.name} ({param.type.value}, {required_str}){default_str}{enum_str}: {param.description}")
...
Response:"""
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if json_match:
                    ...
                    logger.error(f"No JSON object found in LLM response: {llm_response}")
                    return {}
            else:
                logger.error(f"Error calling LLM API: {response.status_code} - {response.text}")
                return {}
    ...
    def _convert_parameter_types(self, tool: BaseTool, params: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        for name, value in params.items():
            ...
            try:
                ...
                if param_type == ToolParameterType.INTEGER and not isinstance(value, int):
                ...
                elif param_type == ToolParameterType.FLOAT and not isinstance(value, float):
                ...
                logger.error(f"Error converting parameter {name} value {value} to {param_type}: {e}")
