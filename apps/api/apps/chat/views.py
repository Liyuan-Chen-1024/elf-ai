    ...
    def send_message(self, request, pk=None):
        ...
        with transaction.atomic():
            ...
            assistant_message = Message.objects.create(
                conversation=conversation, role="assistant", content="Thinking..."
            )

        # First check if this query can be handled by agent tools
        system_prompt = get_system_prompt()
        is_agent_query, agent_response = agent_service.handle_chat_query(
        ...
        try:
            ...
            if response.status_code == 200:
                response_data = response.json()
                assistant_content = response_data.get(
                    "response",
        ...
        def event_stream():
            try:
                ...
                try:
                    ...
                    logger.error(
                        f"Error processing agent query: {str(e)}", exc_info=True
                    )
                ...
                recent_messages = (
                    ...
                    .order_by("created_at")
                    .exclude(id=assistant_message.id)[:5]
                )
                    ...
                    if response.status_code == 200:
                        ...
                        try:
                            (
                                is_agent_query,
                                agent_response,
                            ) = agent_service.handle_chat_query(
                        ...
                        if (
                            ...
                            and user_message.id
                            == conversation.messages.earliest("created_at").id
                        ):
                            try:
                                ...
                                title_response = requests.post(
                                    url, json=title_payload, timeout=10
                                )
                                ...
                                logger.error(
                                    f"Error generating title: {str(e)}", exc_info=True
                                )
