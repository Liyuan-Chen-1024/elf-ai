...
class ConversationAdmin(admin.ModelAdmin):
    list_display = ('id', 'title', 'user', 'created_at', 'updated_at', 'message_count', 'is_deleted')
    ...
    def get_urls(self):
        urls = super().get_urls()
        custom_urls = [
            path('dashboard/', self.admin_site.admin_view(self.dashboard_view), name='chat_dashboard'),
        ]
    ...
    def dashboard_view(self, request):
        ...
        # Messages by role
        messages_by_role = Message.objects.filter(is_deleted=False).values('role').annotate(
            count=Count('id')
        ).order_by('role')
        
        # Recent conversations
        recent_conversations = Conversation.objects.filter(is_deleted=False).order_by('-updated_at')[:10]
    ...
    def clear_all_messages(self, request, queryset):
        ...
        self.message_user(request, f"Cleared {message_count} messages from {queryset.count()} conversations.")
    clear_all_messages.short_description = "Clear all messages from selected conversations"

@admin.register(Message)
class MessageAdmin(admin.ModelAdmin):
    list_display = ('id', 'conversation_link', 'role_badge', 'short_content', 'created_at', 'is_edited', 'is_deleted')
    list_filter = ('role', 'is_edited', 'is_deleted', 'created_at')
    search_fields = ('content', 'conversation__title')
    readonly_fields = ('created_at', 'updated_at', 'is_edited', 'edited_at', 'conversation')
    date_hierarchy = 'created_at'
